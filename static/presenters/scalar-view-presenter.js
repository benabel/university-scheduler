/* scalar-view-presenter.js — Transforms data to viewModel for generated scalar views
 *
 * Pure logic — testable without browser, SF, or DOM
 * Pure functions: (data, view) -> { lanes, axis } for generated views
 * Contains byIndex / detached / peakLoad logic
 */

const SLOT_MINUTES = 60;

/**
 * Build scalar view payload for list variables
 * @param {Object} data - Raw data
 * @param {Object} view - View definition from uiModel
 * @returns {Object|null} - Payload for list variable timeline or null
 */
export function presentListView(data, view) {
...
/**
 * Build scalar view payload for scalar variables
 * @param {Object} data - Raw data
 * @param {Object} view - View definition from uiModel
 * @returns {Object|null} - Payload for scalar variable timeline or null
 */
export function presentScalarView(data, view) {
	const entities = data[view.entityPlural] || [];
	const facts = data[view.sourcePlural] || [];
...
function buildSlotAxis(slotCount) {
	...
	return {
		...
		initialViewport: {
			startMinute: 0,
			endMinute: Math.min(normalizedSlots, 12) * SLOT_MINUTES,
		},
	};
}
